...
export function SamsungShop() {
  ...
  const loadProducts = async () => {
    try {
      ...
      const samsung = allProducts.filter(product => 
        product.brand.toLowerCase() === "samsung" && product.is_active
      );
      setProducts(samsung);
      
      // Set max price based on actual products
      const maxPrice = Math.max(...samsung.map(p => p.price));
      setPriceRange([0, Math.min(maxPrice, 300000)]);
    } catch (err) {
      setError("Failed to load Samsung products");
      console.error("Error loading Samsung products:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const categories = ["all", "flagship", "mid-range", "budget", "foldable"];
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-20">
        <div className="container mx-auto px-4 py-8 text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Samsung</h1>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20">
      <div className="container mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Samsung</h1>
          <p className="text-gray-600">
            Explore innovative Samsung smartphones with advanced features
          </p>
        </div>
          ...
          <div className="lg:w-3/4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="relative">
                  <select
                    ...
                    className="appearance-none bg-white border border-gray-300 rounded-lg px-4 py-2 pr-10 shadow-sm focus:ring-2 focus:ring-green-500 focus:outline-none"
                  >
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-2xl shadow-sm">
                <p className="text-gray-500 text-lg mb-4">No Samsung products found matching your filters</p>
                <button
                  onClick={() => {
                    setSelectedCategory("all");
                    setPriceRange([0, 300000]);
